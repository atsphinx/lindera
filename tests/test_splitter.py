...
def test_dictionary(dict_type: str):
    ...
    src_dir = Path(t.__file__).parent
    sys_dict = t.SystemDictionary(dict_type, lindera_version)  # ty: ignore[invalid-argument-type]
    assert (
        sys_dict.local_path
        == src_dir / "_dict" / lindera_version / f"lindera-{dict_type}"
    )
    sys_dict.fetch()
